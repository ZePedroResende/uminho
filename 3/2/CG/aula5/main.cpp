...
void drawGrid(unsigned int h, unsigned int w, float *vertexB) {
  for(unsigned int i= 0; i<h; h++){
    for(unsigned int j=0; j<w; j++ ){
      int index = i*w +j;
      vertexB[3*index + 0] = (float) j;
      vertexB[3*index + 1] = (float) i;
      vertexB[3*index + 2] = 0.0f;
    }
  }
}
...
// Colocar aqui load da imagem que representa o mapa de alturas
  ...
  imageData = ilGetData();
  size = 3 *tw * th;
  float *vertexB = new float[3 *tw * th];
  drawGrid(th,tw,vertexB);
  glEnableClientState(GL_VERTEX_ARRAY);
  glGenBuffers(1, buffers);
  glBindBuffer(GL_ARRAY_BUFFER,buffers[0]);
  glBufferData(GL_ARRAY_BUFFER,3*tw*th, vertexB, GL_STATIC_DRAW);
